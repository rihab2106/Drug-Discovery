...
parser.add_argument('--target_prop', help='target properties', nargs='+', type=float)
...
target_prop_str = ' '.join(str(p) for p in args.target_prop)
target_prop = np.array([[float(p) for p in target_prop_str.split()] for _ in range(args.batch_size)])
